...
public class AlunoCtrl {
  ...
  public ResponseEntity<Aluno> insert(@RequestBody Aluno aluno) {
    ...
    try {
      ...
      headers.add("message", Message.get("0134"));
    } catch (AlunoException e) {
      status = HttpStatus.BAD_REQUEST;
      headers.add("message", Message.get(e.getMessage()));
    } catch (Exception e) {
      status = HttpStatus.INTERNAL_SERVER_ERROR;
      headers.add("message", Message.get("0135"));
    }
  ...
  public ResponseEntity<Void> delete(@PathVariable Integer id) {
    ...
    try {
      business.delete(id);
      headers.add("message", Message.get("0136"));
    } catch (Exception e) {
      status = HttpStatus.INTERNAL_SERVER_ERROR;
      headers.add("message", Message.get("0137"));
    }
  ...
  public ResponseEntity<List<Aluno>> findByCurso(@PathVariable Optional<String> str) {
    ...
    List<Aluno> list = business.findByCurso(str.get());
    if (list.size() == 0) {
      status = HttpStatus.NO_CONTENT;
      headers.add("message", Message.get("0140"));
    }
    return new ResponseEntity<List<Aluno>>(list, headers, status);
  }
  ...
  public ResponseEntity<Aluno> update(@RequestBody Aluno aluno) {
    ...
    try {
      aluno = business.insert(aluno);
      headers.add("message", Message.get("0138"));
    } catch (AlunoException e) {
      status = HttpStatus.BAD_REQUEST;
      headers.add("message", Message.get(e.getMessage()));
    } catch (Exception e) {
      status = HttpStatus.INTERNAL_SERVER_ERROR;
      headers.add("message", Message.get("0139"));
    }
    return new ResponseEntity<Aluno>(aluno, headers, status);
  }
