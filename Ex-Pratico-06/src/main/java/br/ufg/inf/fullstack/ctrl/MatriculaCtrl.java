...
public class MatriculaCtrl {
  ...
  public ResponseEntity<List<Matricula>> findAll() {
    ...
    if (list.size() == 0) {
      ...
      headers.add("message", Message.get("0123"));
    }
  ...
  public ResponseEntity<List<Matricula>> findByName(@PathVariable Optional<String> str) {
    HttpHeaders headers = new HttpHeaders();
    HttpStatus status = HttpStatus.OK;

    List<Matricula> list = business.findByNmAluno(str.get());
    if (list.size() == 0) {
      status = HttpStatus.NO_CONTENT;
      headers.add("message", Message.get("0112"));
    }
    return new ResponseEntity<List<Matricula>>(list, headers, status);
  }
  ...
  public ResponseEntity<Matricula> update(@RequestBody Matricula matricula) {
    ...
    try {
      matricula = business.insert(matricula);
      headers.add("message", Message.get("0129"));
    } catch (MatriculaException e) {
      status = HttpStatus.BAD_REQUEST;
      headers.add("message", Message.get(e.getMessage()));
    } catch (Exception e) {
