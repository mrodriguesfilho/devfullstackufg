...
public class AlunoCtrl {
	...
	public ResponseEntity<List<Aluno>> findAll() {
		...
		if(list.size() == 0) {
			...
			headers.add("message", Message.get("0611"));
		}
	...
	public ResponseEntity<List<Aluno>> findByName(@PathVariable Optional<String> str){
		...
		List<Aluno> list = business.findAlunoByName(str.get()); 
		if(list.size() == 0) {
			status = HttpStatus.NO_CONTENT;
			headers.add("message", Message.get("0611"));
		}
	...
	@GetMapping(value="/curso/{str}")
	public ResponseEntity<List<Aluno>> findByCurso(@PathVariable Optional<String> str){
		HttpHeaders headers = new HttpHeaders();
		HttpStatus status = HttpStatus.OK;
		
		List<Aluno> list = business.findAlunoByCurso(str.get()); 
		if(list.size() == 0) {
			status = HttpStatus.NO_CONTENT;
			headers.add("message", Message.get("0611"));
		}
	...
	public ResponseEntity<Void> delete (@PathVariable Integer id){
		...
		try {
			business.delete(id);
			headers.add("message", Message.get("0607"));
		}catch (Exception e) {
			status = HttpStatus.INTERNAL_SERVER_ERROR;
			headers.add("message", Message.get("0608"));
		}
	...
	public ResponseEntity<Aluno> update(@RequestBody Aluno aluno){
		...
		try {
			aluno = business.insert(aluno);
			headers.add("message", Message.get("0605"));
		} catch (AlunoException e) {
			status = HttpStatus.BAD_REQUEST;
			headers.add("message", Message.get(e.getMessage()));			
		} catch (Exception e) {
			status = HttpStatus.INTERNAL_SERVER_ERROR;
			headers.add("message", Message.get("0606"));	
		}
		return new ResponseEntity<Aluno>(aluno, headers, status);
	}
